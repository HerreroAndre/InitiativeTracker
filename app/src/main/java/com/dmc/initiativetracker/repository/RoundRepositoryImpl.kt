...
class RoundRepositoryImpl(
    ...
    override suspend fun deleteExpiredStatuses() {
        statusDao.deleteExpiredNormalStatuses()
        statusDao.deleteExpiredConcentrationGroups()
    }

    override suspend fun decrementStatusesForCharacterTurn(
        characterId: Long,
        tickTiming: StatusTickTiming
    ) {
        statusDao.decrementNormalStatusesForCharacter(
            characterId = characterId,
            tickTiming = tickTiming.name
        )

        statusDao.decrementConcentrationGroupsOwnedByCharacter(
            characterId = characterId,
            tickTiming = tickTiming.name
        )
    }
