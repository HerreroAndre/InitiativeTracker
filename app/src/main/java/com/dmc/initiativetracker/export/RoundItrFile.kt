...
object RoundItrFile {
    ...
    fun writeItrToCache(
        ...
        val safeName = fileName
            .ifBlank { "ronda.itr" }
            .let { if (it.endsWith(".itr")) it else "$it.itr" }

        val dir = File(context.cacheDir, "shared_itr").apply {
            mkdirs()
        }
    ...
    private fun writePayloadToStream(
        context: Context,
        output: OutputStream,
        payload: RoundItrExportPayload
    ) {
        ...
        ZipOutputStream(output).use { zip ->
            ...
            payload.images.forEach { image ->
                val bytes = resolver.openInputStream(image.imageUri.toUri())
